...
class DocumentRepository:
    ...
    async def get_analytics(self):
        ...
        verified_query = select(func.count(Document.id)).where(Document.status == "VERIFIED")
        rejected_query = select(func.count(Document.id)).where(Document.status == "REJECTED")
